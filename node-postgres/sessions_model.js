...
const getUsers = (body) => {
  ...
  const createSpot = (body) => {
    return new Promise(function(resolve, reject) { 
      ...
      pool.query('INSERT INTO spots (spot_name, location, image, details, security, obstacles, challenges) VALUES ($1, $2, $3, $4, $5, $6, $7)', [spot_name, location, image, details, security, obstacles, challenges], (error, results) => {
        if (error) {
          console.log(error);
          reject(error)
        }
  ...
  const modifyProfile = (body) => {
    return new Promise(function(resolve, reject) { 
      const { image, age, stance, location, board, trucks, wheels, bearings, shoes, favourite_clip, user_name } = body
  ...
  const createPost = (body) => {
    return new Promise(function(resolve, reject) { 
      ...
      pool.query('INSERT INTO posts (post_name, post_body, user_name) VALUES ($1, $2, $3) RETURNING *', [post_name, post_body, user_name], (error, results) => {
        if (error) {
          console.log(error);
          reject(error)
        }
